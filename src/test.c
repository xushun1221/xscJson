...
static void test_parse_object() {
    ...
    EXPECT_EQ_STRING("o", json_get_object_key(&v, 6), json_get_object_key_length(&v, 6));
    {
        ...
        for (i = 0; i < 3; i++) {
            ...
            EXPECT_EQ_TRUE('1' + i == json_get_object_key(o, i)[0]);
...
static void test_parse_invalid_string_char() {
    TEST_ERROR(JSON_PARSE_INVALID_STRING_CHAR, "\"\x01\""); // "(0x01)" 单个char值
...
static void test_copy() {
    ...
    json_init(&v2);
    json_copy(&v2, &v1);
    EXPECT_EQ_TRUE(json_is_equal(&v2, &v1));
    json_free(&v1);
    json_free(&v2);
}
...
static void test_move() {
    ...
    EXPECT_EQ_TRUE(json_is_equal(&v3, &v1));
    json_free(&v1);
    json_free(&v2);
    json_free(&v3);
}
...
static void test_swap() {
    ...
    json_swap(&v1, &v2);
    EXPECT_EQ_STRING("World!", json_get_string(&v1), json_get_string_length(&v1));
    EXPECT_EQ_STRING("Hello",  json_get_string(&v2), json_get_string_length(&v2));
    json_free(&v1);
    json_free(&v2);
}
...
static void test_access_number() {
    ...
    json_init(&v);
    json_set_string(&v, "a", 1);
    json_set_number(&v, 123.456);
    EXPECT_EQ_DOUBLE(123.456, json_get_number(&v));
    json_free(&v);
}
...
static void test_access_array() {
    ...
    EXPECT_EQ_TRUE(json_get_array_capacity(&a) > 8);
    json_shrink_array(&a);
...
static void test_access_object() {
    ...
    for (j = 0; j <= 5; j += 5) {
        ...
        for (i = 0; i < 10; i++) {
            ...
            EXPECT_EQ_TRUE(index != JSON_KEY_NOT_EXIST);
    ...
    EXPECT_EQ_TRUE(json_get_object_capacity(&o) > 8);
    ...
    EXPECT_EQ_TRUE(pv != NULL);
    EXPECT_EQ_STRING("Hello", json_get_string(pv), json_get_string_length(pv));
